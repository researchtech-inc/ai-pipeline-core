...
The inactivity gate has two thresholds: a strict ``inactivity_seconds`` while
the model is producing text, and a relaxed ``tool_call_inactivity_seconds``
that applies after a tool-call-only chunk. Provider built-in tools (e.g.
OpenAI's ``web_search``) surface lifecycle progress as Chat Completions
``delta.tool_calls`` keepalives; the strict gap is too tight for the long
upstream phases between them. Text content snaps the gate back to strict
mode so a model that genuinely stalls after speaking is still killed quickly.
...
class StreamWatchdog:
    ...
    def on_content(self, *, text_tokens: int = 0, tool_call_tokens: int = 0) -> None:
        """Record visible content from the latest streaming chunk.

        ``text_tokens`` and ``tool_call_tokens`` partition the chunk's visible
        signal. A mixed chunk (text > 0) classifies as text — the strict
        inactivity gate applies on the next ``check()``. A tool-call-only
        chunk flips the relaxed gate on; the next text chunk flips it back.
        """
        total = text_tokens + tool_call_tokens
        ...
        self._last_chunk_was_tool_call = text_tokens == 0 and tool_call_tokens > 0
