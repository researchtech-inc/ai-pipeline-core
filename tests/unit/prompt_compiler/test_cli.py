...
def test_resolve_spec_class_explicit_ref() -> None:
    ...
    cls = _resolve_spec_class("tests.unit.prompt_compiler.test_api:PlainSpec", Path.cwd())
...
def test_resolve_spec_class_explicit_ref_not_a_spec() -> None:
    with pytest.raises(ValueError, match="is not a PromptSpec subclass"):
        _resolve_spec_class("tests.unit.prompt_compiler.test_api:ApiRole", Path.cwd())
...
def test_main_render(capsys: pytest.CaptureFixture[str]) -> None:
    ret = main(["render", "tests.unit.prompt_compiler.test_api:PlainSpec"])
    assert ret == 0
    out = capsys.readouterr().out
    assert "# Role" in out
    assert "# Task" in out
...
def test_main_render_no_input_documents(capsys: pytest.CaptureFixture[str]) -> None:
    ret = main(["render", "--no-input-documents", "tests.unit.prompt_compiler.test_api:PlainSpec"])
    assert ret == 0
    out = capsys.readouterr().out
    assert "# Role" in out
...
@pytest.mark.ai_docs
@pytest.mark.xdist_group("prompts_compile_cwd")
...
def test_resolve_spec_class_auto_discover() -> None:
    ...
    # PlainSpec is defined in tests.unit.prompt_compiler.test_api which is already imported,
