...
from ai_pipeline_core._llm_core import CoreMessage, LLMRequest, TextContent
...
class _GenerateRecorder:
    ...
    async def __call__(self, request: LLMRequest, *args: object, **kwargs: object) -> ModelResponse[str]:
        _ = (args, kwargs)
        self.requests.append(_normalize_core_messages(request.messages))
        if not self._responses:
            raise AssertionError("LLM generate was called more times than expected in the replay integration test.")
