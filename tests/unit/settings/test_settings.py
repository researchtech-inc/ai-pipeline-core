...
""")
    ...
    def test_doc_summary_model_empty_string_is_none(self) -> None:
        """Empty DOC_SUMMARY_MODEL disables summary generation."""
        with patch.dict(
            os.environ,
            {"OPENAI_BASE_URL": "http://localhost:4000", "OPENAI_API_KEY": "test-key", "DOC_SUMMARY_MODEL": ""},
            clear=True,
        ):
            s = Settings(_env_file=None)

        assert s.doc_summary_model is None
    ...
    def test_doc_summary_model_mapping_coerces_to_aimodel(self) -> None:
        """Programmatic settings accept an AIModel-shaped mapping."""
        s = Settings(
            openai_base_url="http://localhost:4000",
            openai_api_key="test-key",
            doc_summary_model={"name": DEFAULT_TEST_MODEL.name, "cache_ttl": 1},
            _env_file=None,
        )
