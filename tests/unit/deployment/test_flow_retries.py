...
class TestNonRetriableErrorClassification:
    ...
    def test_llm_core_terminal_error_classifies_as_provider_error(self) -> None:
        """Bare ``_llm_core.TerminalError`` (no framework PipelineCoreError base)
        must still classify as PROVIDER_ERROR via the LLMError branch."""
        from ai_pipeline_core._llm_core.exceptions import TerminalError as _CoreTerminalError
        from ai_pipeline_core.deployment._helpers import _classify_error
        from ai_pipeline_core.deployment._types import ErrorCode

        assert _classify_error(_CoreTerminalError("provider error")) == ErrorCode.PROVIDER_ERROR
