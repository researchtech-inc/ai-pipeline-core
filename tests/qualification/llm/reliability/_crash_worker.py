"""Helper subprocess for qualification crash-resilience coverage."""
...
class _CrashDeployment(PipelineDeployment[FlowOptions, _CrashResult]):
    """Deployment used by crash-resilience qualification tests."""
...
async def _run_worker(db_path: Path, mode: str) -> int:
    ...
    deployment_task = asyncio.create_task(_run_deployment(database=database, prefect_flow_run_id=prefect_flow_run_id))
