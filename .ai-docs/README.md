...
# ai-pipeline-core v0.21.3 — API Reference
...
class DatabaseReader(Protocol):
    ...
    async def list_deployments(self, limit: int, *, status: str | None=None, root_only: bool=False) -> list[SpanRecord]: ...
    ...
    async def list_orphaned_deployment_roots(self, *, older_than: datetime, limit: int=1000) -> list[SpanRecord]: ...
